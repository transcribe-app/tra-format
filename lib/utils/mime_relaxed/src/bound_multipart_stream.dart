...
void _expectByteValue(int val1, int val2) {
  if (val1 != val2) {
    throw MimeMultipartException('Failed to parse multipart mime 1 (${val1} != ${val2})');
  }
}

void _expectWhitespace(int byte) {
  if (byte != char_code.sp && byte != char_code.ht) {
    throw MimeMultipartException('Failed to parse multipart mime 2 (${byte})');
  }
}
...
class BoundMultipartStream {
  ...
  BoundMultipartStream(this._boundary, Stream<List<int>> stream, bool lfonly) {
    _lfonly = lfonly;
    _controller
  ...
  void _parse() {
    ...
    while (
      ...
      // debugPrint("// MIME: ${_index}. ${byte} -> ${_state} ${_boundaryIndex} | ${String.fromCharCode(byte)}");
      switch (_state) {
        case _startCode:
        ...
        case _headerFieldCode:
          if (byte == char_code.colon) {
            ...
            if (!_isTokenChar(byte)) {
              throw MimeMultipartException('Invalid header field name (${byte})');
            }
            _headerField.add(_toLowerCase(byte));
          }
        ...
        case _lastBoundaryDash2Code:
          ...
          if(_lfonly && _index == _buffer.length-1){
            // last "\n" relaxed can be ignored
            _state = _doneCode;
          }
      ...
      _index++;
      // if(_index%10000 == 0){debugPrint("// MIME: progress: ${_index} ..${_state}");}
    }
    // debugPrint("// MIME: part PARSED: ${_index} ..${_state}");
